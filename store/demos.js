...
export const actions = {
  ...
  async create ({ commit, state, rootGetters }) {
    try {
      ...
      const demo = await this.$axios.post(
  ...
  async clone ({ commit, state, rootGetters, dispatch }, data) {
    try {
      ...
      if (data.playbookType !== 'Custom Bot') {
        data.widgetId = null
      }
      const demo = await this.$axios.post(
        `${process.env.API_URL}/demo_settings/clone`,
        {
          user: userId,
          id: data.demoId,
          name: data.demoName
  ...
  async update ({ commit, state, rootGetters, dispatch }, data) {
    try {
      ...
      const demo = await this.$axios.put(
        `${process.env.API_URL}/demo_settings/${state.selected}`,
        {
          name: data.name,
          settings: {
            email: data.email,
  ...
  async delete ({ commit, state, rootGetters, dispatch }) {
    try {
      ...
      const demo = await this.$axios.delete(
        `${process.env.API_URL}/demo_settings/${state.selected}`
      )
      dispatch('get')
  ...
  activate ({ commit, state }) {
    ...
    this.app.router.push({
      query: { ...this.app.router.currentRoute.query, demo: state.activated }
    })
    setTimeout(() => {
      location.reload()
    }, 1000)
